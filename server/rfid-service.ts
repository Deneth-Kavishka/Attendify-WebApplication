...
class RFIDService extends EventEmitter {
  ...
  init(wss: WebSocketServer, broadcast: (data: any) => void): void {
    ...
    wss.on('connection', (ws: WebSocket, req: IncomingMessage) => {
      ...
      const isRFIDDevice = userAgent.includes('ESP32') || userAgent.includes('Arduino') || req.url?.includes('rfid');
   ...
   */
  private handleDeviceMessage(ws: WebSocket, socketId: string, message: any): void {
   ...
   */
  private handlePlainTextMessage(ws: WebSocket, socketId: string, message: string): void {
    const device = this.devices.get(socketId);
    if (!device) return;
   ...
   */
  private handleClientMessage(ws: WebSocket & { rfidSubscribed?: boolean }, message: any): void {
    switch (message.type) {
      case 'subscribe_rfid':
   ...
   */
  private handleDeviceRegistration(ws: WebSocket, socketId: string, data: any): void {
    const device = this.devices.get(socketId);
    if (!device) return;
   ...
   */
  private async handleCardDetection(ws: WebSocket, socketId: string, data: any): Promise<void> {
    try {
      const device = this.devices.get(socketId);
      if (!device) return;
      
      console.log(`🔍 Card detected: ${data.cardId} from device: ${device.id || socketId}`);
  ...
  private handleRFIDCommand(ws: WebSocket, data: any): void {
    ...
    console.log(`📡 RFID Command sent: ${command} to device: ${targetDevice.id || targetSocketId}`);
   ...
   */
  private async processAttendance(cardData: CardData, student: Student): Promise<void> {
    try {
      ...
      const enrollment = await this.checkStudentEnrollment(student.student_id, activeClass.class_id);
      ...
      if (attendanceRecord) {
        console.log(`✅ Attendance recorded for ${student.name} in ${activeClass.subject_name}`);
  ...
  private async findActiveClass(): Promise<Class | null> {
    try {
      ...
      const activeClass = stmt.get(currentDay, currentTime, currentTime) as Class | undefined;
   ...
   */
  private async checkStudentEnrollment(studentId: string, classId: string): Promise<any> {
    try {
      const stmt = database.prepare(`
...
export { rfidService };
  ...
  handleDeviceRegistration(socket, data) {
    const deviceInfo = {
      ...
      capabilities: data.capabilities || 'card_detection',
      registeredAt: new Date(),
      lastSeen: new Date(),
      status: 'connected'
    };
    
    this.devices.set(socket.id, deviceInfo);
    ...
    // Broadcast device status to monitors
    this.broadcastToMonitors('rfid_status', {
      deviceId: data.deviceId,
      status: 'connected',
      scanning: false,
      timestamp: new Date()
   ...
   * Handle RFID card detection
   */
  async handleCardDetection(socket, data) {
    try {
      const device = this.devices.get(socket.id);
      if (!device) {
        console.log('❌ Card detection from unregistered device');
        return;
      }
      
      console.log(`🔍 Card detected: ${data.cardId} from device: ${device.id}`);
      ...
      const cardData = {
        ...
        studentId: student?.student_id || null,
        studentName: student?.name || 'Unknown Student',
        classId: null, // Will be determined by active class
        className: null,
        status: 'present'
      };
      
      // If student found, process attendance
      if (student) {
        await this.processAttendance(cardData, student);
      } else {
        console.log(`⚠️  Unknown card: ${data.cardId}`);
      ...
      // Broadcast card detection to all monitors
      this.broadcastToMonitors('rfid_card_detected', cardData);
      
      // Send acknowledgment to device
      socket.emit('card_processed', {
        cardId: data.cardId,
        status: 'processed',
        studentFound: !!student,
        timestamp: new Date()
  ...
  handleHeartbeat(socket, data) {
    ...
    // Broadcast heartbeat to monitors
    this.broadcastToMonitors('rfid_heartbeat', {
      deviceId: device.id,
      status: data.status || 'connected',
      timestamp: new Date()
    });
  ...
  handleRFIDCommand(socket, data) {
    ...
    if (deviceId) {
      ...
      // Use first available device
      targetDevice = Array.from(this.devices.values())[0];
    }
    ...
    targetDevice.socket.emit('rfid_command', {
      command: command,
      timestamp: new Date()
    });
    
    console.log(`📡 RFID Command sent: ${command} to device: ${targetDevice.id}`);
  ...
  handleDeviceDisconnection(socket) {
    ...
    if (device) {
      ...
      // Broadcast disconnection to monitors
      this.broadcastToMonitors('rfid_status', {
        deviceId: device.id,
        status: 'disconnected',
        scanning: false,
   ...
   * Look up student by RFID card ID
   */
  async lookupStudentByCard(cardId) {
    try {
      const stmt = database.prepare(`
        SELECT s.student_id, s.name, s.email, s.rfid_card_id,
               s.status, s.enrollment_year
        FROM students s 
        WHERE s.rfid_card_id = ? AND s.status = 'active'
      `);
      
      const student = stmt.get(cardId);
      return student;
    } catch (error) {
      console.error('❌ Error looking up student by card:', error);
      return null;
   ...
   * Process attendance for detected card
   */
  async processAttendance(cardData, student) {
    try {
      // Find active class for current time
      const activeClass = await this.findActiveClass();
  ...
  async findActiveClass() {
    try {
      ...
      const activeClass = stmt.get(currentDay, currentTime, currentTime);
      return activeClass;
    } catch (error) {
      console.error('❌ Error finding active class:', error);
      return null;
   ...
   * Record attendance in database
   */
  async recordAttendance(studentId, classId, timestamp, status, method) {
    try {
      const attendanceDate = timestamp.toISOString().split('T')[0]; // YYYY-MM-DD
  ...
  broadcastToMonitors(event, data) {
    if (this.io) {
      this.io.to('rfid_monitors').emit(event, {
        ...data,
        type: event,
        timestamp: data.timestamp || new Date()
      });
